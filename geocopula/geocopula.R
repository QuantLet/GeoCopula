...
libraries = c("foreign", "gstat", "spacetime", "fossil", "tseries", "mvtnorm", "xts")
...
for(i in 1:nrow(na_indi)){
  ...
  vvd4t[na_indi[i,1],na_indi[i,2]]=0.5*(vvd4[na_indi[i,1]-1,na_indi[i,2]]+vvd4[na_indi[i,1]+1,na_indi[i,2]])
...
# WLS as, initial parameter, Chapter5.2
# set multiple initial value for WLS estimation
a=c(1e-10,0.5,1)
b=c(1e-20,1e-5,0.1,0.3)
...
pre_data = pre_data
##pre_data=pre_y # in case of WCL
#pre_data=pre_wls  # in case of WLS
...
for(i in 1: ncountry)
{
   plot(y4[,i],type="l",xlab="month",ylab="inflation rate",col="blue")
   lines(d3[,i],type="l",xlab="month",ylab="inflation rate",col="red")
...
for(i in 1: ncountry)
{
   ...
   plot( time,y4[,i], type="l",xlab="",ylab="",col="blue", lwd = 2, lty = 1, ylim = c(-0.8, 1.2))
   lines(time, d3[,i],type="l",xlab="",ylab="",col="red", lwd = 2, lty = 2, ylim = c(-0.8, 1.2) )
