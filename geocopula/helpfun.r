...
Kernel =function (h, u, nu, a, b, beta, sigma) {  #same as in GeoCopula
  ...
  y[idx2] = sigma*2*beta/c1^nu/c2/gamma(nu)*(b/2*h[idx2]*(c1/c2)^0.5)^nu*besselK(b*h[idx2]*(c1/c2)^0.5,nu)
  return(y)
}

kernel=function(h,u,nu,par,sigma){ #another kernel.Only change the parameter of function
...
vv_p=function(data,location,start=c(1998,1),end=c(2008,5),keep=NULL){
  ...
  time=seq(ISOdate(start[1],start[2],28), ISOdate(end[1],end[2],28), by = "1 months")
  ...
  for(i in 1:nrow(na_indi)){
    ...
    vv[na_indi[i,1],na_indi[i,2]]=0.5*(vv[na_indi[i,1]-1,na_indi[i,2]]+vv[na_indi[i,1]+1,na_indi[i,2]])
...
fit.vv=function(vv,a=0.01,b=0.005,beta=0.1,nu){
  ...
  fit.model=function(data0,par0,nu,sigma){
    ...
    weightFun[1]=0
    gammaMod=sigma-sapply(1:nrow(data0),function(i) Kernel(data0$avgDist[i],data0$timelag[i],nu,a,b,beta,sigma))
    sum=sum(weightFun*(data0$gamma-gammaMod)^2)
    return(sum)
  }
  vv[1,2:3]=0
  k=optim(par=par0,fit.model,data0=vv,nu=nu,sigma=sigma,method="L-BFGS-B",lower=c(1e-10,1e-10,0.1))
  k
}
...
fit.count=function(vv,a,b,beta){
  ...
  k=sapply(1:nrow(op),function(i) fitv(vv,a=a[op[i,1]],b=b[op[i,2]],beta=beta[op[i,3]]))
  xabc=function(x,abc){
    abc[x]
  }
  op[,1]=xabc(op[,1],a)
...
# score calculation for JCEF estimation, suitable for setting in this paper
...
bivariate.score.fast.wrap=function(dat,pair,a,b,beta,sigma,nu){
  ...
  d_dr2_beta=b*h*0.5*((a^2*u^2+1)/(a^2*u^2+beta))^(-0.5)*(a^2*u^2+1)*(-1)/(a^2*u^2+beta)^2
  d_dr2_a=b*h*0.5*((a^2*u^2+1)/(a^2*u^2+beta))^(-0.5)*2*a*u^2*(beta-1)/(a^2*u^2+beta)^2
  ...
  if(h>0){
    d_beta=d_beta0/gamma(nu) *(dr2/2)^nu *besselK(dr2, nu)+
      dr1/gamma(nu) *nu*(dr2/2)^(nu-1)*d_dr2_beta/2*besselK(dr2, nu)+
      dr1/gamma(nu) *(dr2/2)^nu *(nu/dr2*besselK(dr2,nu)-besselK(dr2,nu+1))*d_dr2_beta
    d_a=d_a0/gamma(nu) *(dr2/2)^nu *besselK(dr2, nu)+
      dr1/gamma(nu) *nu*(dr2/2)^(nu-1)*d_dr2_a/2*besselK(dr2, nu)+
      dr1/gamma(nu) *(dr2/2)^nu *(nu/dr2*besselK(dr2,nu)-besselK(dr2,nu+1))*d_dr2_a
    d_b=dr1/gamma(nu) *nu*(dr2/2)^(nu-1)*dr2/2/b*besselK(dr2, nu)+
      dr1/gamma(nu) *(dr2/2)^nu*(nu/dr2*besselK(dr2,nu)-besselK(dr2,nu+1))*dr2/b
    d_beta=d_beta*2
    d_a=d_a*2
    d_b=d_b*2
  }
  ...
  score_corrcoef = corrcoef * rfun1 - corrcoef * rfun1 * 
    rfun1 * (mu1 * mu1 + mu2 * mu2 - 2 * corrcoef * mu1 * 
               mu2) + rfun1 * mu1 * mu2   
  score = matrix(c(score_corrcoef * d_a, 
                   score_corrcoef * d_b, score_corrcoef *d_beta), nrow = 3, ncol = 1)
  return(score)
}


# sum of scores from relevant samples, this is a function from GeoCopula,
# but also can be used to WCL estimate. In WCL setting score means likelihood
bivariate.score.all=function(dat,pt,pf,pft,a,b,beta,sigma,nu){
  score_spatial=sapply(1:nrow(pf),function(i) bivariate.score.fast.wrap
                       (dat,pf[i,],a,b,beta,sigma,nu))
  score_temporal=sapply(1:nrow(pt),function(i) bivariate.score.fast.wrap
                        (dat,pt[i,],a,b,beta,sigma,nu))
  score_s_and_p=sapply(1:nrow(pft),function(i) bivariate.score.fast.wrap
                       (dat,pft[i,],a,b,beta,sigma,nu))
  ...
  return(list(meanscore_s= meanscore_s, meanscore_t = meanscore_t, 
              meanscore_t_s = meanscore_t_s)
  )
}
...
logh_1=function(dat,pair,a,b,beta,sigma,nu){
  ...
  value=-0.5*log(1-k*k)-0.5*(mu1*mu1-2*k*mu1*mu2+mu2*mu2)/(1-k*k)
  value
}
logh_sum=function(x,nu,sigma,dat,pt,pf,pft){
  ...
  score_spatial=sapply(1:nrow(pf),function(i) logh_1
                       (dat,pf[i,],a,b,beta,sigma,nu))
  score_temporal=sapply(1:nrow(pt),function(i) logh_1
                        (dat,pt[i,],a,b,beta,sigma,nu))
  score_s_and_p=sapply(1:nrow(pft),function(i) logh_1
                       (dat,pft[i,],a,b,beta,sigma,nu))
  l=sum(score_spatial)+sum(score_temporal)+sum(score_s_and_p)
  return(-l)
}

# three pair producer of tree kinds, time(pt), space(pf) and spatiotemporal(pft)
...
optim_p=function(opti,dat0,pt,pf,pft,sigma=1){
  ...
  opt=function(par,dat0, nu ,sigma,pt,pf,pft){
    est <- optim(par, logh_sum, dat=dat0, nu=nu,sigma=sigma, 
                 pt=pt,pf=pf,pft=pft, control = list(maxit = maxit, trace = trace, 
                                                     reltol = reltol))
...
optim_s=function(opti,dat0,nu=0.5,sigma=1,weight){
  ...
  opt=function(par,dat0, nu ,sigma,weight){
    est <- optim(par, Q.fun, dat=dat0, nu=nu,sigma=sigma, 
                 pt=pt,pf=pf,pft=pft,approach="boot",weight=weight, control = list(maxit = maxit, trace = trace, 
                                                                                   reltol = reltol))
    return(c(est$par,est$value))
  }
...
c_p=function(data,tp, start){
 #continuous prediction
  ...
  #optimum
  a=c(1e-10,0.1) # 
  b=c(1e-10,1e-5,0.001,0.1)
  beta=c(1e-10,0.001,0.01,0.1)
  #a=1e-10#
  #b=1e-5# weile jiandan
  #beta=1e-10#
  ...
  pft=PFT[[3]]
  reltol=0.01# gai 
  system.time(est_o<-optim_p(as.matrix(opti_10[1,]),dat0=d,pt,pf,pft))
...
#continuous esitimation for WSL
c_w=function(data,tp, start){ #continuous prediction
  d=data[1:tp,]
  nc=ncol(d)
  nr=nrow(d)
  end=c(1:2)
...
# estimation function for bootstrip in JECF
c_p1=function(data){ #continuous prediction
  tp=nrow(data)
  d=data[1:tp,]
  nc=ncol(d)
  nr=nrow(d)
...
ar_p=function(mis,method="in",tp,p=3){
  if(method=="in"){
    ...
    for(i in tp){
      ...
      for(j in 1:p){
        y[i-tp[1]+1,1]=y[i-tp[1]+1,1]+fit4$coef[j]*(d3_forcas[i-j]-fit4$coef[p+1])
      }
    }
    y=y[-1,]
    y
...
#best nu
nu_p=function(data,tp,start,location){ #continuous prediction
## data = d4
## tp   = 62
## startyear = start[1]
  d=data[1:tp,]
...
pre_term1=function(data,tlag,par){
  ...
  for(i in 1:nc){
    for(j in 1:tlag){
      for(k in 1:nc){
        cov_xy[i,(j-1)*nc+k]=kernel(dd[i,k],j*30.42742,nu,par.fit,sigma)
      }
    }
  }
...
pre_single1=function(data,tlag,mis,par.fit){
  ...
  for(j in 1:tlag){
    for(k in 1:nc){
      cov_xy[1,j*nc+k-1]=kernel(dd[index_c,k],j*30.42742,nu,par.fit,sigma)
    }
  }
  ...
  for(i in 1:(nc-1)){
    for(j in 1:(nc-1)){
      cov_x[i,j]=kernel(dd[keep[i],keep[j]],0,nu,par.fit,sigma)
    }
  }
  for(i in 1:(nc-1)){
    for(j in 1:(tlag)){
      for(k in 1:nc){
        cov_x[i,j*nc+k-1]=kernel(dd[keep[i],k],j,nu,par.fit,sigma)
      }
    }
  }
  for(i in 1:(nc-1)){
    for(j in 1:(tlag)){
      for(k in 1:nc){
        cov_x[j*nc+k-1,i]=kernel(dd[keep[i],k],j,nu,par.fit,sigma)
      }
    }
  }
  for(i in 1:nc){
...
plot_result=function(pre_data,mis=4,m1="term",m2="n",m3="in",tp=c(55:125))

# 使用if做一个plot function，把其他的都弄好
  
{
  if(m1=="single"){
    ...
    if(m2=="m"| m2=="r"){
      y3[,1]=m_a_m(as.matrix(y3[,1]),1)}
  ...
  }else{yy_g=matrix(0,length(tp)-1,8)
        pa0=as.matrix(pre_data[1,][1:3])
        for(i in tp[2]:tp[length(tp)]){
          k=i-tp[1]+1
          if(m3=="in"){pa=pa0
        ...
        colnames(yyy)=c("predicted","real")
        y3=yyy
        if(m2=="m"| m2=="r"){y3[,1]=m_a_m(as.matrix(y3[,1]),1)}
  }
  
  if(m2=="m"|m2=="n"){
    png(filename = paste(m3,m1,m2,mis,".png",sep="_"),width = 500, height = 300)
    plot(y3[,1],type="l",ylim=c(-3,3),xaxt = "n",xlab="month",ylab="value",col="blue")
    axis(1, at=seq(6,71,7),labels=rownames(y3)[seq(6,71,7)]) # Transformed Value in N(0,1) Inflation rate
    par(new = TRUE)
    plot(y3[,2],type="l",col="red",xaxt = "n",axes = FALSE,ylim=c(-3,3),xlab="",ylab="")
    dev.off()
    re=y3
  }else{y4=y3
        ...
        png(filename = paste(m3,m1,m2,mis,".png",sep="_"),width = 500, height = 300)
        plot(y4[,1],type="l",ylim=c(-1,5),xaxt = "n",xlab="month",ylab="inflation rate",col="blue")
        axis(1, at=seq(6,70,7),labels=rownames(y3)[seq(6,70,7)]) # Transformed Value in N(0,1) Inflation rate
        par(new = TRUE)
        plot(y4[,2],type="l",col="red",xaxt = "n",axes = FALSE,ylim=c(-1,5),xlab="",ylab="")
        dev.off()
        re=y4
  }
  re
...
plot_v=function(data,vv,par){
  ...
  k$par[4]=1 #sigma=1
  data0=vv
  gammaMod=k$par[4]-sapply(1:nrow(data0),function(i) Kernel(data0$avgDist[i],data0$timelag[i],nu,k$par[1],k$par[2],k$par[3],k$par[4]))
  vv1=vv
  vv1$gamma=gammaMod
  plot(vv1,wireframe=T,xlab=list("distance (km)", rot=30),
       ylab=list("time lag (days)", rot=-35),
       scales=list(arrows=F, z = list(distance = 5)),zlim=c(0,1.2))
}
# moving average smoother

m_a_m=function(d,n){ #use 2n as detrend
  k=nrow(d)
  for(i in (n+1):(k-n))
  { sum=d[i-n,]
    for(j in (i-n+1):(i+n)){
